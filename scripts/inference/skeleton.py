...
# Modified by Yen from pyskl:demo/demo_skeleton.py
# move this file to pyskl/demo/ before running the code
import argparse
import cv2
import mmcv
import numpy as np
...
def parse_args():
    ...
    parser.add_argument('video', help='image file name in the data/input folder')
    ...
    parser.add_argument(
        '--det-checkpoint',
        default=('../models/faster_rcnn_r50_fpn_1x_coco-person_20201216_175929-d022e227.pth'),
    ...
    parser.add_argument(
        '--pose-checkpoint',
        default=('../models/hrnet_w32_coco_256x192-c78dce93_20200708.pth'),
...
def main():
    args = parse_args()
    video_path = '../data/input/' + args.video
    ...
    MODIFICATION BELOW
    '''
    # cv2.imwrite(args.out_filename, vis_frames[0])
    
    # save keypoints of the forearms in the first frame
    # the first forearm is formed of keypoints 7,9
    # the second forearm is formed of keypoints 8,10
    keypoints = pose_results[0][0]['keypoints']
    keypoints_arm1 = keypoints[[7,9]]
    keypoints_arm2 = keypoints[[8,10]]
    keypoints_arm1 = keypoints_arm1[:, :2]
    keypoints_arm2 = keypoints_arm2[:, :2]
    keypoints_arm1 = keypoints_arm1.astype(int)
    keypoints_arm2 = keypoints_arm2.astype(int)
    
    # create a folder in ../data/intermediate/arm_vectors/ with the name of the video
    os.makedirs('../data/intermediate/arm_vectors/'+args.video[:-4], exist_ok=True)

    # create a folder in ../data/intermediate/products/ with the name of the video
    # save the keypoints of the forearms in the first frame
    np.save('../data/intermediate/arm_vectors/'+args.video[:-4]+'/arm1.npy', keypoints_arm1)
    np.save('../data/intermediate/arm_vectors/'+args.video[:-4]+'/arm2.npy', keypoints_arm2)

    # np.save('keypoints_arm1.npy', keypoints_arm1)
    # np.save('keypoints_arm2.npy', keypoints_arm2)
    img = cv2.imread(frame_paths[0])
    vector_arm1 = keypoints_arm1[1] - keypoints_arm1[0]
    vector_arm2 = keypoints_arm2[1] - keypoints_arm2[0]
    endpoint_arm1 = keypoints_arm1[0] + vector_arm1 * 10000
    endpoint_arm2 = keypoints_arm2[0] + vector_arm2 * 10000
    cv2.line(img, tuple(keypoints_arm1[0]), tuple(endpoint_arm1), (0, 255, 0), 2)
    cv2.line(img, tuple(keypoints_arm2[0]), tuple(endpoint_arm2), (255, 0, 0), 2)
    
    cv2.imwrite('../data/intermediate/arm_vectors/'+args.video[:-4]+"/arms.jpg", img)
