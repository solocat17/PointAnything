...
def pose_inference(args, frame_paths, det_results):
    ...
    for f, d in zip(frame_paths, det_results):
        ...
        # 提取前臂关节点信息
        for person_pose in pose:
            keypoints = person_pose['keypoints']
            if keypoints.shape[0] > 10:  # 确保关键点数组有足够的大小
                forearm_info = keypoints[[7, 9, 8, 10]]
                forearm_keypoints.append(forearm_info)
            
        prog_bar.update()
    return ret, forearm_keypoints
...
def main():
    ...
    pose_results, forearm_keypoints = pose_inference(args, frame_paths, det_results)
    torch.cuda.empty_cache()
    ...
    if GCN_flag:
        ...
        num_keypoint = 17
        keypoint = np.zeros((num_person, 1, num_keypoint, 2), dtype=np.float16)
        keypoint_score = np.zeros((num_person, 1, num_keypoint), dtype=np.float16)
    ...
    vis_frame = vis_pose_result(pose_model, frame_paths[0], pose_results[0])
    cv2.putText(vis_frame, action_label, (10, 30), FONTFACE, FONTSCALE, FONTCOLOR, THICKNESS, LINETYPE)

    output_path = args.out_filename
    cv2.imwrite(output_path, vis_frame)

    # 保存前臂关节点信息
    forearm_output_path = osp.splitext(args.out_filename)[0] + '_forearm_keypoints.npy'
    np.save(forearm_output_path, forearm_keypoints)
    for i, frame in enumerate(original_frames):
        for keypoints in forearm_keypoints:
            for x, y, score in keypoints:
                cv2.circle(frame, (int(x), int(y)), 3, (0, 255, 0), -1)
        forearm_image_path = osp.splitext(args.out_filename)[0] + f'_forearm_{i:06d}.jpg'
        cv2.imwrite(forearm_image_path, frame)
